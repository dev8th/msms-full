...
@endphp
	...
	<table cellspacing="0" border="1">
		...
		<tr>
			...
			<th>Admin</th>
			<th>Klien</th>
			<th colspan="2">Alamat Pengirim</th>
			<th>Total</th>
			<th>Total Diskon</th>
		...
		<tr>
			...
			<td align="center" valign=middle class="middle text-align-center">{{$l->sender_first_name." ".$l->sender_middle_name." ".$l->sender_last_name}}<br>Reference : <?php echo $l->reference!=""?App\Http\Controllers\Controller::getReferenceFullName($l->reference):"-"?></td>
			<td colspan="2" align="center" valign=middle class="middle text-align-center">'{{$l->sender_phone}}<br>{{$l->sender_address}}</td>
			<td align="center" valign=middle class="middle text-align-center">{{round(App\Http\Controllers\Controller::hitungBeratWare($l->warehouse_id,2,$l->mismass_invoice_id),2)}} Kg<br>{{App\Http\Controllers\Controller::hitungItemWare($l->warehouse_id,2,$l->mismass_invoice_id)}} Item<br>{{App\Http\Controllers\Controller::hitungCbmWare($l->warehouse_id,2,$l->mismass_invoice_id)}} CBM</td>
        ...
        <tr>
			...
			<th>No.Resi</th>
			<th>Penerima</th>
			<th colspan="6">Alamat Penerima</th>
			<th>Berat</th>
        </tr>
        @php
            $num2=1;
        ...
        @foreach($list2 as $dl)
            ...
            <tr>
                ...
                <td colspan="6" align="center" valign=middle class="middle text-align-center">'{{$dl->cons_phone}}<br>{{$dl->cons_address.", ".$dl->cons_sub_district.", ".$dl->cons_district.", ".$dl->cons_city.", ".$dl->cons_prov.", ".$dl->cons_postal_code}}</td>
                <td align="center" valign=middle class="middle text-align-center">{{round($dl->weight,2)}} Kg<br>{{$dl->item}} Item<br>{{round($dl->cbm,2)}} CBM</td>
            </tr>
            @php
                $num2++;
            @endphp
        ...
        <tr>
            <td bgcolor="#ffff00" class="bg-yellow" colspan="10"></td>
        </tr>
        @php
            $num++;
        @endphp
